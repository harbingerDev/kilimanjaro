...
test('Combined Test', async ({ request }) => {
  ...
  await test.step(`Make request for API:2 and check status code `, async () => {
          const response2 = await request.get('https://ebs-v80-qa-api.fischeridentitydev.com/api/v1/runtime/users', {
    headers: {"Authorization":`Bearer ${token}`,"X-Fii-UserId":"-81126430295779360995295669317416234544"},
  });

const temp = response2.status(); 

    await test.step(`Expected Status Code: 200  Received Status Code: ${temp}`, async () => {

      expect(response2.status()).toBe(200);

      console.log('Status code of response2 is:', response2.status());
